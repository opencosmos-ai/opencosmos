...
Eight repositories, split by **rights and invitation** rather than by topic — the
reasoning is recorded in [ADR 0018](decisions/0018-the-commons-and-the-applications-live-in-separate-repositories.md).
The organization holds the commons and the site that serves it; the personal
applications were peeled back to `shalomormsby/` on 18 September 2026.
...
| [opencosmos-ai/opencosmos-ui](https://github.com/opencosmos-ai/opencosmos-ui) | `@opencosmos/{ui,tokens,mcp,constellation}`, OpenCosmos Studio | MIT |
...
**Index ownership**, which is the part that matters operationally:

| | [knowledge](https://github.com/opencosmos-ai/knowledge) | [cosmo](https://github.com/opencosmos-ai/cosmo) | this repository |
|---|---|---|---|
| **Owns in Upstash** | the `knowledge/` prefix and both Redis graph keys | the `kaizen/` prefix | **nothing** |
| **Runs with** | `npm run …` | `npm run …` | `pnpm …` |

This repository has no writer to the shared index at all — the last one left with
`packages/ai/` in September. It reads through Upstash at runtime and receives the
corpus as files only at build time.
...
The Spark/Flame/Hearth tiers, the Stripe integration behind them, and the
Substack/Circle benefit provisioning were **removed on 19 September 2026**. Paid
