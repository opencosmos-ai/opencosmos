...
## How this changelog relates to the others

**One change, one changelog — the one that owns the decision.** Seven work histories exist across four repositories; this is the canonical statement of which owns what, and the others carry a scope note pointing back here.
...
1. **Log where the decision was made, not where the file landed.** The ghost-button bug was *found* by playing Xensō and *fixed* in the design system: the full account is in opencosmos-ui's changelog, and Xensō's carries one sentence saying playing found it. A code change is not automatically this repo's entry.
2. **A cross-repo change is logged once in full and once in a sentence.** The sentence links to the full account. Two full accounts is the failure this table exists to prevent.
3. **Never restate a layer that already owns the content.** Every changelog here carries its own scope table naming what lives elsewhere — provenance, principles, worklists, ADRs. A changelog is a *dated spine that points into those*, never a second copy.
4. **Entries are milestones, not commits.** `git log` holds every change.
5. **Counts are a snapshot on the entry's date**, never a table anyone maintains. Generators and run output hold live numbers; a hand-kept copy of a derivable list is the failure mode that killed two separate hand-kept tables in the taoteching repo.
6. **Newest first. One dated heading, a sentence of context, then bullets that link out.**
...
- **Cosmo's Xensō module moves to `cosmo/modules/XENSO_MODULE.md`.** [`next.config.mjs`](next.config.mjs) read the new path first and the old one as a fallback, so the live module was never absent while the two repos merged; the fallback was removed once both had. **The module is now required:** [`fetch-content`](scripts/fetch-content.mjs) and [`next.config.mjs`](next.config.mjs) both fail the build if it is missing or empty, as they already did for the system prompt — without it, xenso mode would quietly run as plain Cosmo. **When Xensō leaves this repo, remove both checks** (the `XENSO_MODULE` line in `next.config.mjs` and the module check in `fetch-content`'s cosmo `verify`), or every build fails. Both carry a comment saying so, and the fetch error names them.
...
- **Deploy:** the Vercel project's Root Directory changes from `apps/web` to the repository root at merge. `vercel.json` drops `installCommand` and sets `buildCommand` to `pnpm build`. Dropping it too turned out to expose a stale dashboard override, `pnpm turbo run build --filter=web`, which `vercel.json` had been masking. The first build after the Root Directory change failed on it, so the repo owns the build command again.
...
The commons no longer lives in a personal namespace. `opencosmos` and `taoteching` now sit under [opencosmos-ai](https://github.com/opencosmos-ai), which is the structural answer to a question a personal namespace cannot answer well: *what happens to my contribution if the owner moves on?* An organisation can answer it. A personal account cannot.

- **Both repositories transferred, everything intact.** `opencosmos-ai/opencosmos` keeps its 5 stars, its default branch, its 22,967 KB of history, **all 8 Actions secrets**, and both workflows active. `opencosmos-ai/taoteching` keeps its CC0 licence and history. GitHub redirects every old URL.
- **Three Vercel projects followed the transfer automatically, and that was not luck.** `opencosmos` (→ opencosmos.ai), `portfolio` (→ shalomormsby.com) and `ecosystem-creative-powerup` all build from this one repository — a detail the plan originally missed, and the reason the personal portfolio was exposed to this step at all. Because Vercel tracks the **numeric repo ID** (`1116060612`), which a transfer preserves, and because the Vercel GitHub App was installed on the organisation *first*, all three relinked with no manual reconnection. **Installing the app before the transfer is the whole trick**; doing it after would have cut three live sites off from builds simultaneously.
- **117 repository URLs repointed across 83 files**, the bulk of them in `knowledge/iching/`, which cites the Tao Te Ching glossary once per hexagram. Redirects make this cosmetic *today* — but a redirect is not permanent: create a repository at the old name and every link through it silently resolves to the wrong work. The 28 references to `opencosmos-ui`, which has **not** moved, were preserved by a negative lookahead rather than a blanket replace.
- **Organisation secrets are now set once at the org, not per repository** — the six the knowledge-sync workflow needs, visibility `all`, sourced from the Vercel production export so no value had to be re-entered by hand. Every repository created here from now on inherits them.
- **Verified:** opencosmos.ai, opencosmos.ai/library and shalomormsby.com all returning 200 throughout; `pnpm turbo run build --filter=web` green after the URL rewrite. **Not verified:** the other four apps were not rebuilt.
...
`knowledge/iching/` had held sixty-four hexagram files since 2026-08-19 with `render: null` in every one and **no text to render** — the figures, the King Wen order and the cast engine were all verified, and the thing they were substrate *for* was absent. It also held an argument against vendoring the one English translation available, which turned out to be a rule invented locally, stricter than the one it inherits, and never examined. Both are now fixed, and the work generated enough transferable rules along the way to need somewhere to keep them.

- **Five public-domain sources vendored — 361 files, 2.6 MB.** The Chinese base text (周易, all 64, plus the per-hexagram Wings), the five standalone Wings, and **three of the four public-domain translations that exist**: Legge 1882, de Harlez 1889 (in French), McClatchie 1876. Plus the Tao Te Ching glossary's lock table, measured against this text — 36 of its 47 locked terms occur in the Zhouyi. Full account in [knowledge/iching/CHANGELOG.md](knowledge/iching/CHANGELOG.md); rights reasoning and per-source grades in [sources/PROVENANCE.md](knowledge/iching/sources/PROVENANCE.md).
- **The import verifies itself against something outside our own data.** Each Chinese page prints its own trigram decomposition, and the classical line labels independently encode each line's polarity — 九 solid, 六 broken — so 初九 九二 六三 六四 九五 上六 spells `110010` and nothing else. **64 of 64 decompositions and all 384 line values now check against a source that has no idea what our table says.** A transposed row cannot survive it.
- **A Wikisource page that looked proofread was not**, and that is why grades are now machine-checked. Hexagram 32's mainspace page returns a fragment plus *hexagram 31's* footnote; it would have shipped as the best-graded file in the set. The importer now requires every proofread page to yield its full paragraph count and demotes anything short.
- **One source needed an OCR engine that did not exist here.** The McClatchie PDF — which Shalom found on HathiTrust after a sweep had wrongly concluded no digitisation was reachable — has **no text layer at all**: 498 JBIG2 page images and 23 JPEG2000 plates. New [`scripts/xenso/ocr-pdf.swift`](scripts/xenso/ocr-pdf.swift) rasterises each page and runs Apple's Vision framework locally. **No npm dependency was added**, deliberately: a root `pnpm add` regenerates the shared lockfile, which has silently broken sibling apps' Turbopack builds here before.
- **`knowledge/iching/principles/` added — 9 entries**, on the taoteching project's model: `trigger:`-first frontmatter, two-independent-cases before `active`, a generated index. New `pnpm xenso:principles` ([build-iching-principles.ts](scripts/xenso/build-iching-principles.ts)) regenerates it, enforces the evidence threshold, and **verifies every `evidence:` anchor resolves to a real heading** — it caught two dead anchors on its first run. Its first rule is *not to restate* the twenty-seven it inherits.
- **The sharpest of the nine is about this collaborator, not the text.** `never-supply-what-the-source-withheld`: where a source is unreadable, the gap is recorded as a gap and never filled from the model's own knowledge. The 1876 edition is bilingual and its Chinese is vertical columnar type no OCR here can read — and a transcription produced by a language model from those page images would be indistinguishable from that model's memory of the Yijing while wearing an 1876 printing's authority. The parent project's `record-the-fact` makes this argument about human scholars; it reaches the AI collaborator harder, because a scholar's reconstruction is at least signed and datable.
- **None of it is embedded, and that is enforced upstream.** `embed-knowledge.ts` skips `iching/` by name — the fix made on 2026-08-22 after 72 files were found being indexed against their own stated intent. The substrate is a keyed lookup table consulted by a cast, not by similarity.
- **Four new commands**, all under the existing `xenso:` prefix: `xenso:import-iching` (vendor and verify), `xenso:principles`, alongside the existing `seed`/`build`/`check`. Two Swift tools ([ocr-pdf](scripts/xenso/ocr-pdf.swift), [page-ink](scripts/xenso/page-ink.swift)) build on demand and are not part of any pipeline.
- **The changelogs became a system, which is the second half of this entry.** Adding a seventh work history to the ecosystem made the absence of a rule expensive: the root's 2026-08-22 entry already narrates a design-system fix, Xensō's narrates opencosmos PRs, and nothing said which one owned what. **The rule is now written once, above** — *one change, one changelog, the one that owns the decision* — with a table naming all seven and six rules under it. Each satellite changelog carries a scope note pointing back rather than restating it.
- **[taoteching](https://github.com/opencosmos-ai/taoteching) got the changelog it never had** — eleven milestone entries backfilled from that repository's **own dated records** (`WORKLIST.md`, `PLAN.md`, `DISCOVERIES.md`, `process/shaloms-call.md`, the principles' `since:` fields) rather than narrated from commit subjects, cross-checked against its 90 commits. It needed the tightest scope note of any of them: that repo already runs seven documentation layers under an explicit *nothing belongs in two of those* doctrine, so its changelog is defined against `WORKLIST.md` as **the single backward-looking file to its single forward-looking one**.
- **Two dormant changelogs named rather than quietly ignored.** `apps/portfolio/` and `apps/creative-powerup/` are changesets output from a tool no longer installed, last written 2026-02-16, still referencing the retired `@thesage/ui`. They are described in the table as dormant release history and left in place; retiring them is a separate decision.
- **Verified** end to end: `pnpm xenso:import-iching` green across all five sources, `xenso:check-iching` green (bijection, the King Wen pair invariant, the cast engine, and the founding cast still resolving to hexagram 60 → 29), `xenso:principles` green with all 18 evidence anchors resolving, and every relative link in all 361 files checked to resolve. **Not visually verified** — no app surface changed; `apps/web/lib/iching-data.ts` regenerated identically apart from the draft-note text.

## 2026-08-22 — Feature: the quote substrate comes alive, and the Library becomes one place (provenance for all 1,509 · unified index · /knowledge → /library)

The quote pipeline had been stalled since 2026-05-08 with 46 of 1,509 quotes live — 3% of the corpus. Three things were actually holding it, none of them the one written down. The recorded blocker was a sequencing choice ("resume after the constellation visualizer ships"), and **that condition had been satisfied on 2026-08-15**. The real blocker was cost: the Stage 3 pilot spent ~$20 validating 10 quotes once web search and high effort compounded, which extrapolated to roughly **$2,900** for the backlog. The fix — drop web search, use training knowledge only — was written into the plan in May but never applied to the code. And the pilot's own 10 verdicts had **never been merged**: three months of paid-for output sitting unused on disk because `03-merge-validation.ts` was never run against its checkpoint.

- **Merged the stranded pilot first, for free.** Running the existing merge against the May checkpoint cost nothing and exercised the whole `merge → promote → lint → embed` path on real data before spending anything new. It immediately surfaced a latent bug: [`lint.ts`](scripts/normalize-quotes/lint.ts) assumed every pending record was unvalidated, so a `verified`/`attributed` record sitting below the 0.8 promotion bar — the normal resting state after validation — tripped "should be in embeddable pool." Replaced the status-membership check with the two conditions that actually indicate a misplaced record: `rejected` (belongs in `_archive/`) and anything already clearing `meetsPromotionBar()`.
- **Stage 3 now runs on Claude Code subagents, not the API.** New [`02b-checkpoint.ts`](scripts/normalize-quotes/02b-checkpoint.ts) (`pnpm quotes:checkpoint`) queues batches as input files, takes verdicts back validated and idempotently, and reports progress. It writes the **identical checkpoint format**, so `03-merge-validation.ts` consumes either driver's output unchanged and `02-validate-provenance.ts` survives as the API path. Resume state derives from *which quote IDs already have a verdict* rather than a batch counter, so tranches can be resized, re-run, or abandoned midway with no bookkeeping — which mattered: the org spend limit killed roughly a third of agents mid-run across six tranches, and **not one batch of work was lost**, because agents write their verdict file before composing their final message and ingest is all-or-nothing per file. The pilot's tuned prompt (status vocabulary, confidence ladder, no-fabrication rules) is preserved verbatim in [`VALIDATION_PROMPT.md`](scripts/normalize-quotes/VALIDATION_PROMPT.md), web search removed.
- **All 1,509 quotes now carry a provenance verdict** — 250 verified (17.1%), 446 attributed (30.5%), 598 attributed_unverified (40.9%), 120 likely_misattributed (8.2%), 49 apocryphal (3.3%). Embeddable pool **46 → 349**, across 178 author and collective files. The 1,160 still pending are not a backlog: most are untraceable material now correctly *described* as untraceable, which is the outcome the substrate exists to produce.
- **The scholarship is the point.** Correct catches include Hafiz→**Daniel Ladinsky**; "We are what we repeatedly do"→**Will Durant**, not Aristotle; the eight "Gandhi" seven-social-sins fragments→**Frederick Lewis Donaldson's 1925 sermon** (one isn't in the original list at all); "not the strongest of the species"→**Leon C. Megginson** (1963), not Darwin; "I have loved the stars too fondly"→**Sarah Williams** (1868); "Not everything that is faced can be changed"→**James Baldwin**, not Lucille Ball; the C. S. Lewis "prayer changes me" line→**Nicholson's *Shadowlands*** dialogue; two "Sagan" lines→***Contact*** screenplay by Hart and Goldenberg; the Jobs "crazy enough" line→**Think Different campaign copy**.
- **Fixed Cosmo's broken quote deeplinks — root cause was an asymmetry in the prompt.** Passage citations told Cosmo *"Use the token exactly as given; never invent one for a passage that wasn't retrieved."* Quote citations instead handed it a **fill-in-the-blank template** (`[quote: knowledge/quotes/{author-key}.yaml#{quote-id}]`) with no such prohibition — so whenever Cosmo recalled a quote from training rather than retrieval, it did the natural thing and filled the template in, producing a path to a record that does not exist. Those 404'd. Even a *correct* token was rendered with its `#fragment` stripped and pointed at raw YAML on github.com, so it never landed on the quote. Now quotes carry a "Cite as" line like passages do ([`rag.ts`](apps/web/lib/rag.ts)), and [`citations.tsx`](apps/web/app/dialog/citations.tsx) resolves them to `/library/quotes/{bucket}#{id}` with the fragment intact; a target that doesn't parse renders as **no link** rather than a dead one, matching how passage refs already behave.
- **Quotes have human pages.** New [`/library/quotes`](apps/web/app/library/(library)/quotes/page.tsx) index (browse by author or collective, filter by tradition) and per-bucket pages with provenance badges, print sources, `#q_id` anchors, a CSS-only `:target` highlight, and a not-found page that explains itself. Backed by [`lib/quotes.ts`](apps/web/lib/quotes.ts), which reads the YAML through gray-matter's bundled yaml engine rather than adding `js-yaml` as a direct dependency — deliberately avoiding a rewrite of the shared monorepo lockfile.
- **Stage 4 (human review) shipped**, scoped to records where the validator asserts the attribution is *wrong* rather than everything below the bar — an untraceable proverb at 0.2 confidence is already correctly described and needs no decision. [`04-export-review-csv.ts`](scripts/normalize-quotes/04-export-review-csv.ts) → fill in `decision` → [`05-apply-review.ts`](scripts/normalize-quotes/05-apply-review.ts) (`keep | drop | reattribute`); drops land in `_archive/rejected.yaml` as a tombstone so a rejected quote is never silently re-imported. **197 rows are queued for review** at `knowledge/quotes/_review/review-2026-08-21.csv`; 128 name a specific alternative author.
- **The pools became canonical, and quotes got a front door.** With everything validated, `_source/quotes_normalized.jsonl` is no longer the truth — it is the May import, holding none of the verdicts or review decisions since. [`01-jsonl-to-yaml.ts`](scripts/normalize-quotes/01-jsonl-to-yaml.ts) is **retired**: renamed `pnpm quotes:migrate-from-source`, it now refuses without `--i-know-this-wipes` and prints what it would destroy first (it rebuilds both pools from the import, which would silently discard all 1,509 verdicts). Kept rather than deleted because it documents how the corpus was built. New quotes enter through **[`/new-quote`](.claude/skills/new-quote/SKILL.md)** → [`08-add-quote.ts`](scripts/normalize-quotes/08-add-quote.ts) (`pnpm quotes:add`), following the `/groom` → `pnpm knowledge:publish` precedent: the skill does the judgment a CLI can't — parsing free-form input, deduping, inferring category/keywords/context, and *naming the source*, running the quote through `VALIDATION_PROMPT.md` so a well-attested one can go live in the same interaction — while the script stays the single enforcement point for ID allocation, key normalization, and routing. The skill asks via **`AskUserQuestion`** rather than prose — batching the duplicate decision and the source confirmation into one call so Shalom is interrupted once, and not at all when nothing needs deciding. `--json` is the primary interface because quotes are full of apostrophes and em-dashes and shell escaping eventually mangles one. `lint.ts`'s exact-count check became **source coverage**: every imported ID must still live in exactly one pool, but IDs beyond the import are now legal.
- **Two hygiene fixes found along the way.** `walkMd()` in [`embed-knowledge.ts`](scripts/knowledge/embed-knowledge.ts) opted directories out one at a time and only ever named `quotes/`, so **`knowledge/iching/` (72 files) and `knowledge/incoming/` (15 files)** were being indexed against their own stated intent — the latter meaning unreviewed staging content, including in-progress Tao Te Ching translation drafts, was retrievable and citable by Cosmo as corpus. Separately, Tier 1 had left diacritics in some author keys (`andré-breton`), producing quote pages that citations structurally could not address; `routeAuthor()` and the YAML emitters now re-derive keys through `normalizeAuthorKey()`.
- **Closed an asymmetry between the two audiences.** Every quote's keywords were folded into its embedding vector alongside author, tradition, and category — so Cosmo could retrieve by theme — but they were invisible on the page and the index searched author names only. The same metadata, usable by one audience and not the other. Keywords now render as tags on each quote and drive search on the index, so a person can find a quote the way Cosmo does.
- **Cosmo now knows when you're looking at quotes.** `cosmo_context` was written only by `TableOfContents`, which only *document* pages render — so on a quote page the sidebar Cosmo was still holding whatever markdown doc you last opened. You could be reading Marcus Aurelius while Cosmo believed you were in the Dhammapada. Quote pages now publish their own context, and [`api/chat/route.ts`](apps/web/app/api/chat/route.ts) recognizes a quotes YAML path and synthesizes the current document from the bucket — deliberately via the single-bucket reader, since this runs inside the chat request path where `getQuoteBuckets()` would read all 178 files.
- **Then undid the silo that shipping quotes created.** Giving quotes their own reader and route let a data-layer difference (YAML records vs markdown frontmatter) become the information architecture: searching "einstein" on the library returned **zero results** while 11 Einstein quotes sat one URL away. New [`lib/library.ts`](apps/web/lib/library.ts) composes both readers into one `LibraryItem` list and a single `LibraryBrowser` renders a doc card or a quote card off `kind` — the tiles differ, the architecture doesn't. Exploration found the silo was the smaller half of the problem: **`author` was never searched at all** (the card rendered it; the filter covered only title/summary/tags/domain, so "hesse" failed too), and the **`domain` facet is set on just 25/110 docs with 3 of its 9 values being template junk** (`<primary domain or cross>`) rendering as filter pills. Switched the facet to `tradition` — 91/110 docs, 51/178 quote buckets, and the only vocabulary both kinds share, so "stoicism" now returns *Meditations* **and** Marcus Aurelius' quotes. Quote text is searchable now too, and a text match swaps the card's blurb to the *matching* quote and anchors the link to it.
- **`/knowledge` → `/library`.** The UI already said "The Library" everywhere — page title, both index headers, every breadcrumb, and the route group is literally named `(library)`. Only the URL and nav disagreed. Two namespaces meet here and only one moved: the **URL** changed; the **corpus path** (`knowledge/sources/x.md#slug`) did not, because it keys every Upstash chunk id and every citation token Cosmo has ever emitted. Because `preprocessCitations()` runs at *render* time on stored token text rather than a baked URL, **every past conversation and public share link resolves to the new URL with no migration**. Permanent redirects added to `vercel.json`; `/api/knowledge/*` deliberately left alone. The riskiest line was `.github/workflows/knowledge-sync.yml` — `revalidatePath()` on a stale path returns 200 and silently no-ops, so a miss would have served a stale graph for the full ISR window with no error anywhere.
- **Quotes joined the graph properly.** `emitQuoteBlock` hardcoded `source_work: null`, so the constellation's intended `quote → work` **cites** edge had never fired once — every quote hung off its tradition by fallback. New [`09-resolve-source-works.ts`](scripts/normalize-quotes/09-resolve-source-works.ts) matches print sources to corpus works, conservatively (title match **and** author agreement — without the author check, Watts' *Nature, Man and Woman* matches Emerson's *Nature*). All 24 links reviewed by hand. Constellation: **700 nodes/920 edges → 1,009 nodes/1,058 edges**. Also fixed graph nodes that opened GitHub blobs for quotes (stale since the quote pages shipped) and synthesis nodes that 404'd against the non-browsable `wiki/`.
- **Performance.** Merging 178 quote sources into a 110-card index roughly triples the searchable data, so search is two-tier: metadata inline, and the 53KB of quote text as a **static build artifact fetched once on first focus of the search box** — only people who search pay for it. Cards assemble their own search text from fields already in the payload instead of shipping a duplicate pre-joined string, and blurbs are clamped to the three lines the card actually renders. Index page **82KB → 69KB gzipped** with no visible change.
- **Verified** end to end against the running app: **all 349 citable quotes resolve to a page carrying their anchor id** (0 unresolvable, 0 non-200, 0 missing anchors); path traversal and malformed buckets produce no link; an unknown bucket returns 404 with a helpful page. Re-embedded once Upstash's daily write cap reset — **4,070 chunks live, every quote citable by Cosmo**. The intake loop was exercised on a real add (correct ID past the import range, key normalized, status preclassified, lint reconciling as "1509 from the original import, 1 added since", validation queue picking it up) and the test record then removed. All nine `quotes:*` commands smoke-tested; `pnpm quotes:lint` green after every mutation; `tsc --noEmit` clean on `apps/web`. **Not visually verified in a browser** — HTTP-level assertions only (status codes plus asserting quote text, provenance badges, and anchor ids in the rendered HTML).
...
- **Follow-up fix: client never knew it was admin.** First live test failed — Cosmo denied having access at all. Root cause: `GET /api/admin/auth`, which sets the client's `pmMode` state (the gate on ever sending `creativeMode: true`), still only checked the legacy `cosmo_admin` cookie — it was never taught about `ADMIN_EMAIL`. The chat route recognized Shalom as admin server-side, but the client didn't, so `?creative=1` silently sent no creative context. Fixed `GET /api/admin/auth` to also resolve the WorkOS user and check `ADMIN_EMAIL`, and made both email comparisons case-insensitive.
- **Verified** via a full `next build` (all 138 static pages, including `/knowledge/[...slug]`) and green CI `build` checks on [PR #154](https://github.com/opencosmos-ai/opencosmos/pull/154) and [PR #156](https://github.com/opencosmos-ai/opencosmos/pull/156).
...
- **Phase 3 — exemplar few-shot (shipped 2026-06-19).** The first curated exemplar is now injected as a cached system block in both routes (`next.config.mjs` bundles `kaizen/exemplars/cosmo/*.md` into `COSMO_EXEMPLARS`, frontmatter stripped). Lessons set a floor; exemplars set the ceiling. Verified: given a "I feel scattered / disconnected from my why" prompt, Cosmo reproduced the exemplar's *posture* — attune-before-advising, the diagnostic distinction, ending on one question to sit with — while quoting none of its specifics (it reached for a different, corpus-cited grounding instead). Open work: curate a *practical* and a *challenging* exemplar for range, then selection-by-query-type.
- **Verified** on localhost against the live API (BYOK + admin): chat/inception 200, `web_fetch` intact, Cosmo quotes its real lesson verbatim and no longer fabricates one, self-referential queries retrieve the kaizen log, and the exemplar steers voice without parroting.
